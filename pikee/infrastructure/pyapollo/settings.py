...
class ApolloSettingsConfig(BaseSettings):
    ...
    @classmethod
    def validate_app_secret(cls, v: Optional[str], info: ValidationInfo) -> Optional[str]:
    ...
    @classmethod
    def from_env_file(cls: Type["ApolloSettingsConfig"], env_file_path: str, **kwargs: Any) -> "ApolloSettingsConfig":
        ...
        config = SettingsConfigDict(
            env_prefix="APOLLO_", case_sensitive=False, env_file=env_file_path, env_file_encoding="utf-8"
        )

        # Create a new class with the custom config
        CustomSettings = type("CustomApolloSettingsConfig", (cls,), {"model_config": config})
